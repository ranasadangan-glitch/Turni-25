...
-- DROP + CREATE (not CREATE OR REPLACE): the view selects e.*, so any new
-- employees column (e.g. the multi-select arrays in migration 08) shifts the
-- column list and makes CREATE OR REPLACE fail with "cannot change name of
-- view column". Dropping first keeps the migration re-runnable.
