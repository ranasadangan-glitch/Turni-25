...
async function safe(fn) {
  try { return await fn(); }
  catch (e) { logger.error('kpi', 'query error', e); return null; }
}
...
unified_forecast AS (
  -- Forecast from the single source of truth: schedule_forecasts (keyed by
  -- service_key) per (branch, service, date). (The legacy HR forecasts
  -- fallback leg was removed after the backfill made it redundant.)
  WITH sc AS (
...
-- Forecast per (branch, service, date) from schedule_forecasts, summed to a
-- daily total. (The legacy HR forecasts fallback leg was removed post-backfill.)
fc_recon AS (
...
router.get('/', async (req, res) => {
    ...
    const r = await pool.query(`
      ...
      entries_by_branch AS (
        SELECT branch_code,
               ...
               -- vacation & medical breakdowns (by code, since both are 'mal')
               count(*) FILTER (WHERE upper(code) IN ('F','ROL','EXF','PR','PT','ASP','CONG'))::int AS vacation,
        ...
        FROM (
          ...
          UNION ALL
          SELECT se.branch_code, se.shift_code AS code, sk.is_work, sk.is_absence, sk.is_off
      ...
      forecast_by_branch AS (
        ...
        -- Forecast per (branch, service) from schedule_forecasts (the legacy HR
        -- forecasts fallback leg was removed after the backfill).
        SELECT bc AS branch_code, sum(qty)::int AS required FROM sc GROUP BY bc
      )
      SELECT d.branch_code,
             ...
             COALESCE(f.required,0)  AS required
        FROM drivers_by_branch d
        LEFT JOIN entries_by_branch e USING(branch_code)
        LEFT JOIN forecast_by_branch f USING(branch_code)
       WHERE d.branch_code != ''
  ...
  // ── 11. Service summary — per-service forecast vs planned ─────────────────
  // Forecast comes from schedule_forecasts (keyed by service_key). "Planned"
  // has to be derived: a service owns a set of shift codes, and that mapping
  // lives in scheduler_config.services (the source of truth), not in SQL — so
  // we count planned drivers per shift_code here and fold them into services
  // in JS rather than doing JSONB gymnastics in the query.
  result.services = await safe(async () => {
    const brC = branch ? `'${branch.replace(/'/g, "''")}'` : null;

    const fcR = await pool.query(`
      SELECT service_key, COALESCE(sum(qty),0)::int AS qty
        FROM schedule_forecasts
       WHERE schedule_month = date_trunc('month', $1::date)
         AND day_of_month   = EXTRACT(DAY FROM $1::date)::int
         ${brC ? `AND branch_code = ${brC}` : ''}
       GROUP BY 1
    `, [date]);

    const plR = await pool.query(`
      WITH ${cte}
      SELECT shift_code, count(DISTINCT driver_key)::int AS cnt
        FROM unified_entries WHERE is_work GROUP BY 1
    `, [date]);

    const cfgR = await pool.query(
      `SELECT config_value FROM scheduler_config
        WHERE branch_code = $1 AND config_key = 'services' LIMIT 1`,
      [branch || 'DLO1']
    );

    const plannedByCode = {};
    for (const row of plR.rows) plannedByCode[String(row.shift_code)] = row.cnt;
    const fcByKey = {};
    for (const row of fcR.rows) fcByKey[row.service_key] = row.qty;

    const defs = (cfgR.rows[0] && cfgR.rows[0].config_value) || [];
    return defs.map((s) => {
      const planned  = (s.count || []).reduce((sum, c) => sum + (plannedByCode[c] || 0), 0);
      const forecast = fcByKey[s.key] || 0;
      return {
        key: s.key,
        label: s.label || s.key,
        forecast,
        planned,
        delta: planned - forecast,
        coverage: forecast ? Math.round((planned / forecast) * 100) : (planned ? 100 : 0),
      };
    }).filter((s) => s.forecast || s.planned);
  });

  // ── 12. Weekly preview — the 7 days starting at `date` ────────────────────
  result.week = await safe(async () => {
    const brC = branch ? `'${branch.replace(/'/g, "''")}'` : null;
    const r = await pool.query(`
      WITH days AS (
      ...
      -- Forecast per (branch, service, date) from schedule_forecasts, summed to
      -- a daily total. (The legacy HR forecasts fallback leg was removed.)
      fc_all AS (
        SELECT d, sum(qty)::int AS qty FROM fc_sc GROUP BY d
      ),
      pl_hr AS (
        SELECT s.work_date::date AS d,
      ...
      pl_sc AS (
        SELECT (se.schedule_month + (se.day_of_month - 1) * INTERVAL '1 day')::date AS d,
               count(*) FILTER (WHERE sc.is_work)::int    AS planned,
               count(*) FILTER (WHERE sc.is_absence)::int AS absent
          FROM schedule_entries se LEFT JOIN shift_codes sc ON sc.code = se.shift_code
         WHERE (se.schedule_month + (se.day_of_month - 1) * INTERVAL '1 day')::date
               BETWEEN $1::date AND $1::date + 6
           AND se.employee_id IS NULL
           ${brC ? `AND se.branch_code = ${brC}` : ''}
  ...
  // ── 13. Today's employees — who is on, and on what ────────────────────────
  result.today_employees = await safe(async () => {
    const r = await pool.query(`
      WITH ${cte}
      SELECT ud.cognome, ud.nome, ud.branch_code,
             ue.shift_code, ue.is_work, ue.is_absence, ue.is_off
        FROM unified_drivers ud
        LEFT JOIN unified_entries ue ON ue.driver_key = ud.driver_key
       ORDER BY ud.cognome, ud.nome
       LIMIT 300
    `, [date]);
    return r.rows;
  });
