...
if (IS_PROD && (SECRET === DEFAULT_SECRET || SECRET.length < 64)) {
  logger.error('auth', 'FATAL: JWT_SECRET must be set to a strong value (>=64 chars) in production. Refusing to start.');
  logger.error('auth', 'Generate one with:  node -e "console.log(require(\'crypto\').randomBytes(48).toString(\'hex\'))"');
  process.exit(1);
}
if (!IS_PROD && (SECRET === DEFAULT_SECRET || SECRET.length < 64)) {
  logger.warn('auth', 'WARNING: weak/default JWT secret. Set a 64+ char JWT_SECRET before deploying.');
}
