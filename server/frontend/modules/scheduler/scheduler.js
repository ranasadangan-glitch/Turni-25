...
function renderGrid(){
  ...
  drivers.forEach((dr,i)=>{const vflag=consecutiveFlag(dr),viol=Object.keys(vflag).length>0;h+="<tr><td class='stick c-n'>"+(i+1)+"</td><td class='stick c-name'>"+(viol?"<span title='7+ giorni consecutivi senza riposo' style='color:var(--bad)'>⚠️ </span>":"")+esc(dr.cognome)+" "+esc(dr.nome)+"<small>"+esc(dr.filiale)+" · "+esc(dr.service)+"</small></td>";for(const d of visDays){const c=getCode(dr.id,d),cls=codeCls(c);const _w=(c&&typeof cellWarnings==='function')?cellWarnings(dr,d):[];const _wt=_w.length?esc(_w.map(x=>x.reason).join(' · ')):"";h+="<td id='c_"+dr.id+"_"+d+"' class='cell "+(isWend(YM,d)?"wend":"")+(vflag[d]?" viol":"")+(_w.length?" rulewarn":"")+"'"+(_w.length?(" title='⚠ "+_wt+"' style='position:relative'"):"")+" onclick='openCellEdit("+dr.id+","+d+")' ondblclick='openPicker("+dr.id+","+d+")'>"+(c?"<span class='chip' style='background:var(--"+cls+"-bg);color:var(--"+cls+")'>"+esc(c)+"</span>":"")+(_w.length?"<span style='position:absolute;top:-1px;right:1px;font-size:.6rem;color:var(--bad);line-height:1;pointer-events:none'>⚠</span>":"")+"</td>";}h+="<td class='c-tot'>"+workedDays(dr,visDays)+"</td><td class='c-ctr'>"+esc(dr.contratto||"")+"</td></tr>";});
