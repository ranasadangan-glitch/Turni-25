...
(function (global) {
  ...
  // ── Enterprise wiring (all defensive; a missing AppBus/AppCache is a no-op) ──
  // The first path segment names the "resource" a call touches (/absences?… →
  // "absences"). Used to broadcast changes and to invalidate cached reference data.
  function _resourceOf(path) { var p = String(path).replace(/^\//, '').split(/[?\/]/)[0]; return p || ''; }
  // Broadcast a successful write so dependent modules refresh themselves (wire.js),
  // and drop any now-stale reference cache.
  function _emitData(method, path) {
    try {
      var r = _resourceOf(path);
      if (global.AppCache) {
        global.AppCache.invalidate(r);
        if (/config|branch|service|meta|shift|contract/i.test(path)) global.AppCache.invalidate('meta');
      }
      if (global.AppBus) {
        global.AppBus.emit('data:changed', { resource: r, action: method, path: path });
        global.AppBus.emit('data:' + r + ':changed', { action: method, path: path });
      }
    } catch (e) {}
  }
  function _emitErr(method, path, err) { try { if (global.AppBus) global.AppBus.emit('api:error', { method: method, path: path, error: err }); } catch (e) {} }
  // GET wrapped in the TTL cache — for slow-changing reference lists only.
  function cachedGet(key, ttl, path) { return global.AppCache ? global.AppCache.wrap(key, ttl, function () { return req('GET', path); }) : req('GET', path); }
  ...
  async function req(method, path, body, isForm) {
    try {
      ...
      // Broadcast successful writes so the rest of the SPA stays in sync.
      if (method && method !== 'GET') _emitData(method, path);
      return data;
    } catch (err) {
      _emitErr(method, path, err);
      throw err;
    }
  }
  ...
  const Api = {
    ...
    // meta — reference lists are cached (2 min TTL); any /meta write invalidates them.
    branches: () => cachedGet('meta/branches', 120000, '/meta/branches'),
    ...
    branchesMeta: () => cachedGet('meta/branches', 120000, '/meta/branches'),
    ...
    schedulerVersions: (month, branch) => req('GET', '/scheduler/versions?' + new URLSearchParams({ ...(month ? { month } : {}), ...(branch ? { branch } : {}) })),
    schedulerVersion: (id) => req('GET', '/scheduler/versions/' + id),
    saveSchedulerVersion: (v) => req('POST', '/scheduler/versions', v),
    deleteSchedulerVersion: (id) => req('DELETE', '/scheduler/versions/' + id),
    // Scheduling rule engine
    schedulerRules: () => req('GET', '/scheduler/rules'),
    createSchedulerRule: (r) => req('POST', '/scheduler/rules', r),
    updateSchedulerRule: (id, r) => req('PUT', '/scheduler/rules/' + id, r),
    deleteSchedulerRule: (id) => req('DELETE', '/scheduler/rules/' + id),
    ...
    updateTemplate: (id, t) => req('PUT', '/schedules/templates/' + id, t),
    deleteTemplate: (id) => req('DELETE', '/schedules/templates/' + id),
    ...
    updateAbsence: (id, a) => req('PUT', '/absences/' + id, a),
    setAbsenceStatus: (id, status) => req('PATCH', '/absences/' + id + '/status', { status }),
    ...
    documentsAll: () => req('GET', '/documents'),
    ...
    // roles & permissions — role list cached (2 min); any /roles write invalidates it.
    roles: () => cachedGet('roles', 120000, '/roles'),
    updateRolePermissions: (role, permissions) => req('PUT', '/roles/' + role + '/permissions', { permissions }),
    createRole: (body) => req('POST', '/roles', body),
    deleteRole: (role) => req('DELETE', '/roles/' + role),
    ...
    // Stored uploads (documents, disciplinary PDFs) are served at the ORIGIN
    // root "/uploads/…", NOT under "/api" — so they must bypass the API base
    // that fileUrl adds. Token still appended for the ?token= download path.
    uploadUrl: (path) => (localStorage.getItem('turnidsp_api_base') || '') + path + (path.includes('?') ? '&' : '?') + 'token=' + encodeURIComponent(token || ''),
    ...
    xlsxImportWith: (type, file, fields) => {
      ...
      return req('POST', '/xlsx/import/' + type, fd, true);
    },
  };
