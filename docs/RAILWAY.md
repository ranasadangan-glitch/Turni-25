...
> a true SPA (`/` login, `/app` the application shell). No paths point outside
> `server/` anymore.
...
| `FORCE_HTTPS` | `true` — Railway's edge always terminates TLS and sets `X-Forwarded-Proto` correctly, so it's safe to force HTTPS here. Do **not** set this on a VPS until Nginx is confirmed to forward that header (see `docs/VPS.md`), or every request will redirect-loop or fail. |
